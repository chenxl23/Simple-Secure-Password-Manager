...
class AccountRow(ctk.CTkFrame):
    def __init__(self, master, acc_data: dict, idx: int, on_select, on_view, on_edit, on_delete, **kwargs):
        # 🛠️ 将账号行卡片的边框加粗至 2 像素
        super().__init__(master, fg_color=COLORS["bg_card"], border_color=COLORS["border"], border_width=2, corner_radius=12, **kwargs)
        self.idx = idx
        self.acc = acc_data
        self.selected = False
        ...
        ctk.CTkLabel(logo_frame, text=initials, font=ctk.CTkFont(family=APP_FONT, size=16, weight="bold"),
        ...
        ctk.CTkLabel(title_row, text=acc_data["title"], font=ctk.CTkFont(family=APP_FONT, size=13, weight="bold"),
                     text_color=COLORS["text_primary"], anchor="w").pack(side="left")

        cat_text = acc_data.get("category", "默认")
        cat_lbl = ctk.CTkLabel(title_row, text=cat_text, font=ctk.CTkFont(family=APP_FONT, size=10),
                               text_color=COLORS["accent2"], fg_color=COLORS["bg_sidebar"], corner_radius=4, height=16,
                               width=40)
        cat_lbl.pack(side="left", padx=8)

        ctk.CTkLabel(info, text=acc_data["username"], font=ctk.CTkFont(family=APP_FONT, size=11),
                     text_color=COLORS["text_secondary"], anchor="w").pack(fill="x")
        ctk.CTkLabel(self, text="●●●●●●●●", font=ctk.CTkFont(family="Consolas", size=9),
        ...
        IconButton(btn_frame, text="✕", fg_color=COLORS["bg_btn_sec"], hover_color="#FFEBEA",
                   text_color=COLORS["danger"], command=lambda: on_delete(idx)).pack(side="left", padx=2)
...
class NavItem(ctk.CTkFrame):
    def __init__(self, master, icon: str, label: str, active=False, badge=None, command=None, **kwargs):
        ...
        ctk.CTkLabel(row, text=icon, font=ctk.CTkFont(family=APP_FONT, size=15),
                     text_color=COLORS["accent"] if active else COLORS["text_secondary"], width=22).pack(side="left")
        ctk.CTkLabel(row, text=label, font=ctk.CTkFont(family=APP_FONT, size=13, weight="bold" if active else "normal"),
                     text_color=COLORS["text_primary"] if active else COLORS["text_secondary"], anchor="w").pack(
            side="left", padx=6, fill="x", expand=True)

        if badge is not None:
            self.badge_lbl = ctk.CTkLabel(row, text=str(badge), fg_color=COLORS["bg_deep"], text_color=COLORS["accent"],
                                          corner_radius=8, font=ctk.CTkFont(family=APP_FONT, size=10), width=28,
                                          height=18)
...
class CustomInputDialog(ctk.CTkToplevel):
    def __init__(self, parent, title, prompt, show_char="●"):
        ...
        ctk.CTkLabel(inner, text=prompt, font=ctk.CTkFont(family=APP_FONT, size=13, weight="bold"),
                     text_color=COLORS["text_primary"], anchor="w").pack(fill="x", pady=(0, 12))

        # 🛠️ 修复 1：根据传入的 show_char 决定是否遮罩。输入分类时传入 show_char=None 即可显示明文
        self.entry = StyledEntry(inner, width=300, height=38, show=show_char)
...
class LoginWindow(ctk.CTkToplevel):
    def __init__(self, app_instance):
        ...
        # 🛠️ 边框加粗到 2 像素，同时宽高微调到 66，防止内部图标被挤压
        logo_bg = ctk.CTkFrame(inner, width=66, height=66, fg_color=COLORS["bg_input"], border_color=COLORS["accent"],
                               border_width=2, corner_radius=18)
        logo_bg.pack(pady=(10, 0))
        logo_bg.pack_propagate(False)
        ctk.CTkLabel(logo_bg, text="🛡", font=ctk.CTkFont(family=APP_FONT, size=28)).place(relx=0.5, rely=0.5,
                                                                                          anchor="center")

        ctk.CTkLabel(inner, text="密码金库", font=ctk.CTkFont(family=APP_FONT, size=22, weight="bold"),
        ...
        ctk.CTkLabel(inner, text=hint, font=ctk.CTkFont(family=APP_FONT, size=12),
        ...
        if not self.is_first and self.app.db.get("recovery_payload"):
            ctk.CTkButton(inner, text="忘记密码？使用恢复码重置", fg_color="transparent", hover_color=COLORS["bg_card"],
                          text_color=COLORS["text_muted"], font=ctk.CTkFont(family=APP_FONT, size=11),
                          command=self._recover).pack(pady=(12, 0))
...
class PasswordManagerApp(ctk.CTk):
    def __init__(self):
        ...
        self.db = {"salt": "", "verify_token": "", "recovery_salt": "", "recovery_payload": "", "categories": ["默认"],
                   "accounts": []}
        self._rows = []
    ...
    def unlock_app(self):
        ...
        # 🛠️ 加粗 Logo 边框，稍微调大一丁点尺寸防止挤压
        logo_bg = ctk.CTkFrame(brand, width=40, height=40, fg_color=COLORS["bg_input"], border_color=COLORS["accent"],
                               border_width=2, corner_radius=10)
        logo_bg.pack(side="left")
        logo_bg.pack_propagate(False)
        ctk.CTkLabel(logo_bg, text="🛡", font=ctk.CTkFont(family=APP_FONT, size=18)).place(relx=0.5, rely=0.5,
                                                                                          anchor="center")

        brand_text = ctk.CTkFrame(brand, fg_color="transparent")
        brand_text.pack(side="left", padx=8)
        ctk.CTkLabel(brand_text, text="密码金库", font=ctk.CTkFont(family=APP_FONT, size=14, weight="bold"),
                     text_color=COLORS["text_primary"]).pack(anchor="w")
        ctk.CTkLabel(brand_text, text="SafeVault Pro", font=ctk.CTkFont(family=APP_FONT, size=10),
        ...
        self.title_lbl = ctk.CTkLabel(self.topbar, text="全部账号",
                                      font=ctk.CTkFont(family=APP_FONT, size=20, weight="bold"),
                                      text_color=COLORS["text_primary"])
        self.title_lbl.pack(side="left", anchor="center")

        # 🛠️ 修复：1. 边框 border_width 加粗到 2 像素，彻底拒绝边缘被裁剪
        search_frame = ctk.CTkFrame(self.topbar, fg_color=COLORS["bg_card"], border_color=COLORS["border"],
                                    border_width=2, corner_radius=10)
        search_frame.pack(side="left", padx=20, anchor="center")

        ctk.CTkLabel(search_frame, text="🔍", font=ctk.CTkFont(family=APP_FONT, size=13),
                     text_color=COLORS["text_muted"]).pack(side="left", padx=(10, 4), pady=4)

        self.search_var = ctk.StringVar()
        self.search_var.trace("w", self._on_search)

        # 🛠️ 修复：2. 给右侧留下高达 15 像素的安全边距 (padx=(0, 15))
        ctk.CTkEntry(search_frame, textvariable=self.search_var, width=180, height=30, fg_color="transparent",
                     border_width=0, text_color=COLORS["text_primary"], placeholder_text="搜索平台或账号…",
                     placeholder_text_color=COLORS["text_muted"], font=ctk.CTkFont(family=APP_FONT, size=13)).pack(
            side="left", padx=(0, 15), pady=4)
        GradientButton(self.topbar, text="＋  添加账号", height=38, width=130, command=self._add_account).pack(
            side="right", anchor="center")

        filter_bar = ctk.CTkFrame(content, fg_color="transparent")
        filter_bar.pack(fill="x", padx=24, pady=(24, 8))
        ctk.CTkLabel(filter_bar, text="账号列表", font=ctk.CTkFont(family=APP_FONT, size=14, weight="bold"),
    ...
    def _render_sidebar_menu(self):
        ...
        ctk.CTkLabel(self.menu_scroll, text="主菜单", font=ctk.CTkFont(family=APP_FONT, size=10),
                     text_color=COLORS["text_muted"], anchor="w").pack(fill="x", padx=14, pady=(4, 2))

        is_all_active = (self.current_filter_category == "全部")
        self.all_acc_nav = NavItem(self.menu_scroll, "  ⊞", "全部账号", active=is_all_active,
                                   badge=len(self.db["accounts"]), command=lambda: self._filter_by_category("全部"))
        self.all_acc_nav.pack(fill="x")

        NavItem(self.menu_scroll, "  🔒", "锁定金库", command=self._lock).pack(fill="x")

        ctk.CTkLabel(self.menu_scroll, text="自定义分类", font=ctk.CTkFont(family=APP_FONT, size=10),
                     text_color=COLORS["text_muted"], anchor="w").pack(fill="x", padx=14, pady=(16, 2))
        ...
        for cat in self.db["categories"]:
            ...
            NavItem(self.menu_scroll, "  📁", cat, active=is_active, badge=cat_count,
                    command=lambda c=cat: self._filter_by_category(c)).pack(fill="x")

        ctk.CTkFrame(self.menu_scroll, height=1, fg_color=COLORS["border"]).pack(fill="x", padx=10, pady=10)

        # 改为垂直排列，给予按钮充足的横向呼吸空间
        SecondaryButton(self.menu_scroll, text="＋ 新增分类", height=32, font=ctk.CTkFont(family=APP_FONT, size=11),
                        command=self._add_new_category).pack(fill="x", padx=4, pady=(0, 8))
        SecondaryButton(self.menu_scroll, text="✕ 删除分类", height=32, font=ctk.CTkFont(family=APP_FONT, size=11),
                        text_color=COLORS["danger"], command=self._delete_category).pack(fill="x", padx=4, pady=(0, 10))

    def _add_new_category(self):
        """🛠️ 统一弹窗风格：使用自定义浅色输入框替代原生 simpledialog"""
        dialog = CustomInputDialog(self, "新增类别", "请输入新分类的名称：", show_char=None)
        new_cat = dialog.result

        if not new_cat or not new_cat.strip(): return
        new_cat = new_cat.strip()

        if new_cat == "全部" or new_cat in self.db["categories"]:
            messagebox.showwarning("提示", "该分类类别已存在或属于系统保留字！")
            return
    ...
    def _delete_category(self):
        """🛠️ 交互闭环：安全删除自定义分类，并执行账户数据平滑迁移"""
        # 排除系统级核心保留分类，防止底座被抽空
        custom_cats = [c for c in self.db.get("categories", ["默认"]) if c != "默认"]

        if not custom_cats:
            messagebox.showinfo("提示", "当前没有可删除的自定义分类。")
            return
        ...
        ctk.CTkLabel(inner, text="请选择要移除的分类类别：", font=ctk.CTkFont(family=APP_FONT, size=13, weight="bold"),
                     text_color=COLORS["text_primary"], anchor="w").pack(fill="x", pady=(0, 8))

        # 下拉选择框
        cat_combo = ctk.CTkOptionMenu(inner, width=320, height=38, values=custom_cats, fg_color=COLORS["bg_input"],
                                      button_color=COLORS["bg_btn_sec"], text_color=COLORS["text_primary"])
        cat_combo.pack(pady=(0, 16))
        cat_combo.set(custom_cats[0])

        ctk.CTkLabel(inner, text="⚠️ 注意：该分类下的所有账号将自动安全归入【默认】分类，绝不丢失数据。",
                     font=ctk.CTkFont(family=APP_FONT, size=11), text_color=COLORS["warning"], justify="left",
                     wraplength=300).pack(fill="x", pady=(0, 20))

        def confirm_delete():
            target_cat = cat_combo.get()
            if messagebox.askyesno("二次确认", f"确定要彻底删除分类【{target_cat}】吗？"):
                # 1. 核心安全防护：将该分类下的账号迁移到“默认”分类
                for acc in self.db["accounts"]:
                    if acc.get("category", "默认") == target_cat:
                        acc["category"] = "默认"

                # 2. 从分类名册中剔除
                self.db["categories"].remove(target_cat)

                # 3. 如果当前刚好切在被删分类的视图上，重置回“全部”
                if self.current_filter_category == target_cat:
                    self.current_filter_category = "全部"
                    self.title_lbl.configure(text="全部账号")

                self._save_db()
                self._render_sidebar_menu()
                self._refresh_list()
                dialog.destroy()
                messagebox.showinfo("成功", f"分类【{target_cat}】已成功移除，账号已安全迁移。")

        btn_row = ctk.CTkFrame(inner, fg_color="transparent")
        btn_row.pack(fill="x")
        SecondaryButton(btn_row, text="取消", width=80, command=dialog.destroy).pack(side="right", padx=(8, 0))
        GradientButton(btn_row, text="确认删除", width=90, fg_color=COLORS["danger"], hover_color="#D32F2F",
                       command=confirm_delete).pack(side="right")
    ...
    def _view_password_dialog(self, idx):
        ...
        try:
            ...
            messagebox.showerror("错误", "解密失败！"); return
        ...
        logo_icon_bg = ctk.CTkFrame(head, width=44, height=44, fg_color=COLORS["bg_input"], border_color=fg,
                                    border_width=1, corner_radius=12)
        logo_icon_bg.pack(side="left")
        logo_icon_bg.pack_propagate(False)
        ctk.CTkLabel(logo_icon_bg, text=acc["title"][0].upper(),
                     font=ctk.CTkFont(family=APP_FONT, size=18, weight="bold"), text_color=fg).place(relx=0.5, rely=0.5,
                                                                                                     anchor="center")

        t = ctk.CTkFrame(head, fg_color="transparent")
        t.pack(side="left", padx=10)
        ctk.CTkLabel(t, text=acc["title"], font=ctk.CTkFont(family=APP_FONT, size=15, weight="bold"),
                     text_color=COLORS["text_primary"]).pack(anchor="w")
        ctk.CTkLabel(t, text=acc["username"], font=ctk.CTkFont(family=APP_FONT, size=12),
                     text_color=COLORS["text_secondary"]).pack(anchor="w")

        ctk.CTkLabel(inner, text="密码", font=ctk.CTkFont(family=APP_FONT, size=11), text_color=COLORS["text_muted"],
                     anchor="w").pack(fill="x")
        pwd_frame = ctk.CTkFrame(inner, fg_color=COLORS["bg_input"], border_color=COLORS["border_accent"],
                                 border_width=1, corner_radius=10)
        pwd_frame.pack(fill="x", pady=(4, 12))

        show_var = ctk.BooleanVar(value=False)
        # 使用 Consolas 防止点、下划线被裁剪
        pwd_entry = ctk.CTkEntry(pwd_frame, font=ctk.CTkFont(family="Consolas", size=16, weight="bold"),
                                 fg_color="transparent", border_width=0, text_color=COLORS["accent2"], show="●")
        ...
        ctk.CTkCheckBox(pwd_frame, text="显示", variable=show_var, onvalue=True, offvalue=False,
                        command=lambda: pwd_entry.configure(show="" if show_var.get() else "●"),
                        font=ctk.CTkFont(family=APP_FONT, size=11), text_color=COLORS["text_muted"],
                        fg_color=COLORS["accent"], width=60).pack(side="right", padx=8)
    ...
    def _add_account(self):
        ...
        ctk.CTkLabel(inner, text="添加新账号凭证", font=ctk.CTkFont(family=APP_FONT, size=18, weight="bold"),
                     text_color=COLORS["text_primary"]).pack(anchor="w", pady=(0, 20))

        ctk.CTkLabel(inner, text="平台名称", font=ctk.CTkFont(family=APP_FONT, size=13),
                     text_color=COLORS["text_secondary"]).pack(anchor="w")
        ...
        ctk.CTkLabel(inner, text="账号 / 邮箱", font=ctk.CTkFont(family=APP_FONT, size=13),
                     text_color=COLORS["text_secondary"]).pack(anchor="w")
        ...
        ctk.CTkLabel(inner, text="密码", font=ctk.CTkFont(family=APP_FONT, size=13),
                     text_color=COLORS["text_secondary"]).pack(anchor="w")
        ...
        ctk.CTkLabel(inner, text="账号分类归属", font=ctk.CTkFont(family=APP_FONT, size=13),
                     text_color=COLORS["text_secondary"]).pack(anchor="w")
        cat_options = self.db.get("categories", ["默认"])
        e_cat_menu = ctk.CTkOptionMenu(inner, width=400, height=40, values=cat_options,
                                       font=ctk.CTkFont(family=APP_FONT, size=13), fg_color=COLORS["bg_input"],
                                       button_color=COLORS["bg_btn_sec"], button_hover_color=COLORS["bg_card_hover"],
                                       dropdown_fg_color=COLORS["bg_card"],
                                       dropdown_font=ctk.CTkFont(family=APP_FONT, size=13),
                                       text_color=COLORS["text_primary"])
        ...
        GradientButton(inner, text="➕ 确认添加", width=400, height=42,
                       font=ctk.CTkFont(family=APP_FONT, size=14, weight="bold"), command=save).pack(fill="x")
        e_title.focus()
    ...
    def _edit_dialog(self, idx):
        ...
        ctk.CTkLabel(inner, text="修改账号凭证", font=ctk.CTkFont(family=APP_FONT, size=18, weight="bold"),
                     text_color=COLORS["text_primary"]).pack(anchor="w", pady=(0, 20))
        ...
        GradientButton(inner, text="💾  保存修改", width=400, height=42,
                       font=ctk.CTkFont(family=APP_FONT, size=14, weight="bold"), command=save).pack(fill="x")
    ...
    def _recover_password(self, login_win):
        ...
        try:
            ...
            decrypted = [{"title": a["title"], "username": a["username"],
                          "password": old_fernet.decrypt(a["password"].encode()).decode(),
                          "category": a.get("category", "默认")} for a in self.db["accounts"]]
            ...
            self.db["accounts"] = [
                {"title": a["title"], "username": a["username"], "password": self.crypto.encrypt(a["password"]),
                 "category": a["category"]} for a in decrypted]
    ...
    def _refresh_list(self, keyword=""):
        ...
        for idx, acc in enumerate(self.db["accounts"]):
            if self.current_filter_category != "全部" and acc.get("category",
                                                                  "默认") != self.current_filter_category: continue
            if keyword and keyword.lower() not in acc["title"].lower() and keyword.lower() not in acc[
                "username"].lower(): continue

            row = AccountRow(self.list_scroll, acc, idx, on_select=self._select_row, on_view=self._view_password_dialog,
                             on_edit=self._edit_dialog, on_delete=self._delete_account)
            row.pack(fill="x", pady=4)
            self._rows.append(row)

        if not self._rows:
            ctk.CTkLabel(self.list_scroll, text="该分类下空空如也 📭", font=ctk.CTkFont(family=APP_FONT, size=13),
